...
from src.bvtlib.settings import ARTIFACTS_ROOT
from src.bvtlib.settings import ARTIFACTS_NFS_PATH
...
def store_client_artifact(dut, client_file, section, postfix):
    ...
    os.umask(0000)
    run(['mkdir', '-p', parent])
    run(['rsync', ARTIFACTS_CLIENT_PATH+client_file, ARTIFACTS_ROOT+'/'+'/'.join(segments)],
         timeout=3600)
    return destination
