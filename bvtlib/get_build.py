...
def try_get_build_number_branch(dut, timeout=60):
    ...
    match = search(r'build\s*=\s*([0-9]+)', issue)
    if match is None:
...
def get_build(dut, timeout=60):
    """Return tag on dut or None if it could not be determined"""
    try:
        target = run(['cat','/etc/opkg/all-feed.conf'], 
                     timeout=timeout, host=dut)
    except SubprocessError:
        print 'GET_BUILD: /etc/opkg/all-feed.conf method failed'
    else:
        m = search('[a-z\-]+\-+[0-9]{6}\-[a-z0-9\-]+', target)
        if m:
            print 'GET_BUILD: using /etc/opkg/all-feed-conf found tag', \
                m.group(0), 'in', target
            return m.group(0)
    out = get_build_number_branch(dut, timeout=timeout)
    if out is None:
        return None
    return 'cam-oeprod-%s-%s' % out
