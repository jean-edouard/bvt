...
from bvtlib.call_exec_daemon import call_exec_daemon, run_via_exec_daemon, Fault
from bvtlib.have_driver import have_driver
...
from bvtlib.wait_for_windows import wait_for_guest_to_start
from bvtlib.start_vm import start_vm_if_not_running
from bvtlib.reboot_windows_vm import reboot_windows_vm
from bvtlib import autoit_generator
from bvtlib.settings import XENSETUP_PATH_PATTERN, XENCLIENT_TOOLS_PATTERN, \
    TOOLS_LOG_FILE, EXPECTED_MSIS, EXPECTED_SERVICES, AUTOIT_TAR_URL, \
    UNATTENDED_PATH_PATTERN, ARCHIVE_TOOLS_INSTALLED_VHDS
...
from bvtlib.windows_guest import make_tools_iso_available
from bvtlib.domains import domain_address
from bvtlib.exceptions import UnableToInstallTools, ToolsAlreadyInstalled
...
def start_prompt_remover(vm_address):
    ...
    call_exec_daemon('unpackTarball', [AUTOIT_TAR_URL, 'C:\\'], host = vm_address)
...
def tools_install_problems(host, guest):
    ...
    if not have_driver(vm_address, 'USB', 'Citrix'):
        bad.append('Citrix USB driver missing')
...
def launch_installer(dut, guest, vm_address, traditional, tools_iso_drive):
    ...
    for iteration in range(2 if traditional else 1):
        # TODO: find a way to deteremine if asyncrhonous actions during tools 
        # install have finished at this point, since rebooting windows is risky
        # while drivers are still being installed.
        ensure_stable(vm_address, 300, description=guest+' on '+dut)
        print 'INFO: doing reboot', iteration
        reboot_windows_vm(dut, domain['name'])
        ...
        print 'HEADLINE: done reboot', iteration
...
def install_tools(dut, guest):
    ...
    if ARCHIVE_TOOLS_INSTALLED_VHDS:
        soft_shutdown_guest(dut, guest, timeout=600, method="exec_daemon")
