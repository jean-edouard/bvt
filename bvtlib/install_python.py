...
def install_python(dut):
    ...
    run(['scp', '-r', abspath(split(split(__file__)[0])[0])+'/pyxc', dut+'/root'])
    print 'INFO: launching python installer'
    run(['/root/pyxc/pysetup'], host=host, timeout=3600)


TEST_CASES = [
    { 'description': 'Install the python interpreter and the pyxc code',
      'trigger': 'platform ready',
      'function': install_python,
      'command_line_options': ['--python'],
      'arguments' : [('dut', '$(DUT)')]},
]
