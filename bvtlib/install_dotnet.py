...
from bvtlib.settings import DOTNET_4_KEY, DOTNET_4_NAME, DOTNET_4_0_INSTALLER_URL
...
from bvtlib.wait_for_windows import ensure_stable
...
def install_dotnet(dut, vm_address, guest):
    ...
    retry(lambda:call_exec_daemon(
            'fetchFile', [DOTNET_4_0_INSTALLER_URL, 'C:\\dotnet.exe'],
            host=vm_address, timeout=600), 
          timeout=1200, description='download devcon')

    try:
        run_via_exec_daemon(['C:\\dotnet.exe', '/passive'], timeout=3600, 
                            host=vm_address)
    except (error, Fault):
