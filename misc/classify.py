...
import argparse, pymongo, sys, src.bvtlib.mongodb, src.bvtlib.process_result
...
def list_recent_unclassified(mongo, limit=10):
    ...
    for result in unknown_failures(mongo).limit(limit):
        ...
        cmd = ''
        for key in src.bvtlib.process_result.CLASSIFIER_FIELDS:
...
def main():
    ...
    if args.list:
        for doc in mongo.classifiers.find():
            cmd = ''
            for key in src.bvtlib.process_result.CLASSIFIER_FIELDS + ['earliest_build']:
                field = src.bvtlib.process_result.field_name(key)
                arg = '--'+key.replace('-', '_')
                if doc.get(field):
    ...
    if whiteboard == '':
        ...
        for result in unknown_failures(mongo):
            tot += 1
            if src.bvtlib.process_result.check_classifier(
    ...
    for result in unknown_failures(mongo, null_whiteboard=False):
        code = src.bvtlib.process_result.process_result(result, verbose=False, 
                                                    mongo=mongo)
