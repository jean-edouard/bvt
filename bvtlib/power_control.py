...
from bvtlib.settings import DEFAULT_POWER_CONTROL, AMTTOOL, AMTENV
from bvtlib.mongodb import get_autotest
from bvtlib.database_cursor import open_database
...
def set_power_state(dut, state, args):
    ...
    if APC_match is not None:
        ...
        run([AMTTOOL, dut]+args, env=AMTENV, timeout=60, stdin_push='y\n')
        retry(lambda: verify_power_state(dut, state),
              description ='%s reach power state %s' % (dut, state))
...
def get_power_state(dut):
    ...
    if APC_match is not None:
        ...
        out = run([AMTTOOL, dut], env=AMTENV)
